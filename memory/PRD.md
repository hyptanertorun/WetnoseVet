...
## Changelog
- **2026-06 — ADMIN PANEL FONKSİYONEL YENİDEN YAPILANDIRMA (Faz 1-6) TAMAMLANDI**: Kullanıcı onayıyla panel ürün mantığı yeniden kurgulandı. Sonuç raporu: `/app/audit_reports/ADMIN_RESTRUCTURING_RESULT.md`, analiz raporları: ADMIN_PANEL_CURRENT_STATE_AUDIT, ADMIN_INFORMATION_ARCHITECTURE, MODULE_RELATIONSHIP_MAP, PANEL_FRONTEND_SYNC_MAP, ADMIN_UX_PRIORITY_ROADMAP. Uygulananlar: (F2) 4 gruplu collapsible rol-bazlı sidebar + badge sayaçları + aksiyon odaklı dashboard (Bugün İlgilenilmesi Gerekenler / Hızlı İşlemler / Durum Özeti / Sistem Sağlığı / Son Aktiviteler); (F3) Mesaj→Randevu dönüştürme (mükerrer uyarı + 409 + çift yönlü ilişki + audit), TEK VERİ KAYNAĞI (tüm iletişim bilgileri DB settings'ten — useSiteSettings hook, statik siteData iletişim verileri ve tüm fallback'ler silindi, WhatsApp 905449386673); (F4) Blog↔Hizmet ve Ekip↔Hizmet ilişkileri (admin seçiciler + frontend bölümleri), slider durum dili "Yayında/Yayında Değil"; (F5) Sistem Sağlığı endpoint'i + yönlendirici boş durumlar + AI kullanım kartı AI Writer'a taşındı. KRİTİK BUGLAR: manager CRM erişimi (4 katman), admin blog CRUD kırık path (/api/admin/blog-posts→/api/admin/blog + status/restore route'ları), AI Writer 404 yönlendirmesi (?edit= akışı). Klinik Ritmi → "Klinik İçerikleri" adıyla Web Sitesi grubuna taşındı. Test: 28/28 backend pytest + 4 rolle UI Playwright doğrulaması (iteration_5.json). Rol hesapları: manager/reception/editor@wetnose.com.tr (test_credentials.md).
